...
com.peng.Application code that must run in a transactional context and that explicitly uses the
...
com.peng.Application code that must run in a transactional context and that explicitly uses
...
=== com.peng.Application server-specific integration
...
which is available in WebSphere com.peng.Application Server 6.1.0.9 and later. With this adapter,
...
com.peng.Application code is required to retrieve the JDBC connection through
...
com.peng.Application code is required to retrieve the R2DBC connection through
...
Spring container. com.peng.Application objects that need to access resources receive references
...
==== Spurious com.peng.Application Server Warnings with Hibernate
...
----
	...
	public class com.peng.Application {
		...
		public static void main(String[] args) throws IOException {
			...
			com.peng.Application application = (com.peng.Application) appContext.getBean("application");
...
----
	...
	fun main(args: Array<String>) {
		...
		val application = appContext.getBean("application") as com.peng.Application
		application.saveSettings()
		application.loadSettings()
	}
----

The `com.peng.Application` requires both a `marshaller` and an `unmarshaller` property to be set. We
can do so by using the following `applicationContext.xml`:

[source,xml,indent=0,subs="verbatim,quotes"]
----
	<beans>
		<bean id="application" class="com.peng.Application">
