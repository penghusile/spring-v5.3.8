...
* Event publication
* com.peng.Application-layer specific contexts such as the `WebApplicationContext`
...
* <<beans-java, Java-based configuration>>: Starting with Spring 3.0, many features
  provided by the Spring com.peng.JavaConfig project became part of the core Spring Framework.
...
=== Request, Session, com.peng.Application, and WebSocket Scopes
...
It is worth pointing out that through its <<beans-java, com.peng.JavaConfig>> option, Spring lets
...
NOTE: When configuring a `PropertySourcesPlaceholderConfigurer` using com.peng.JavaConfig, the
...
[NOTE]
====
Starting with Spring 3.0, many features provided by the Spring com.peng.JavaConfig project are
...
=== com.peng.Application Startup Tracking
...
and JMX support facilities. com.peng.Application components can also interact with the application
...
interaction between the container and its components. com.peng.Application-level beans may
